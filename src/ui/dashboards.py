...
def _asset_dashboard(asset: str, dfs):
    ...
    if not df_price.empty:
        st.plotly_chart(components.candlestick(df_price, f"{asset} Price"), width="stretch", key=f"{asset_lc}_price")
    if not df_vix.empty:
        st.plotly_chart(components.vix_deviation(df_vix), width="stretch", key=f"{asset_lc}_vix_dev")
    if not df_cot.empty:
        st.plotly_chart(components.cot_index(df_cot, asset=asset), width="stretch", key=f"{asset_lc}_cot_index")
        st.plotly_chart(components.net_positions(df_cot), width="stretch", key=f"{asset_lc}_net_pos")
        st.plotly_chart(components.z_score(df_cot), width="stretch", key=f"{asset_lc}_z_score")
        st.plotly_chart(components.open_interest(df_cot, asset=asset), width="stretch", key=f"{asset_lc}_oi")
...
def macro_dashboard(dfs):
    ...
    overall_min = min([df["date"].min() for df in all_dfs if not df.empty and "date" in df.columns], default=None)
    overall_max = max([df["date"].max() for df in all_dfs if not df.empty and "date" in df.columns], default=None)
    ...
    if not df_btc.empty and not df_spx.empty:
        st.plotly_chart(
            components.rolling_correlation(df_btc, df_spx, window=60, min_periods=20, x_range_min=overall_min, x_range_max=overall_max),
            width="stretch",
            key="macro_corr",
        )
...
def backtesting_dashboard(dfs, btc_min: dt.date, eth_min: dt.date, global_max: dt.date):
    ...
    with col2:
        initial_capital = st.number_input("Стартовый капитал ($)", min_value=1.0, value=float(s.backtest.initial_capital_default), step=10.0)
    with col3:
        fee_pct = st.number_input("Комиссия (%)", min_value=0.0, max_value=1.0, value=float(s.backtest.fee_default) * 100, step=0.05) / 100
    ...
    st.plotly_chart(components.equity_curve_chart(result.equity_curve, initial_capital=initial_capital), width="stretch", key=f"equity_curve_{asset}")
